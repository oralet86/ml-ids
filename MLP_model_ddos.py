...
HYPERPARAMS_FILE = os.path.join(HYPERPARAMS_SAVE_DIR, "best_hyperparameters.json")

# Genelleme testi için ek CSV dosyaları (opsiyonel)
GENERALIZATION_TEST_CSVS = [
    # "/Users/cemresudeakdag/Downloads/archive/Monday-WorkingHours.pcap_ISCX.csv",
    # "/Users/cemresudeakdag/Downloads/archive/Thursday-WorkingHours-Morning-WebAttacks.pcap_ISCX.csv",
]
...
def build_model(hp, input_dim):
    """Hyperparameter tuning için model builder"""
    model = keras.Sequential()
    model.add(keras.layers.Input(shape=(input_dim,)))
    
    # Katman sayısını optimize et (2-5 arası)
...
def preprocess_and_model_single_csv(csv_path: str, label_column: str = "Label", test_size: float = 0.2, k_folds: int = 5, random_state: int = 42, verbose: bool = False, generalization_csvs: list = None):
    # Veriyi oku
    if not verbose:
    ...
    X_test_scaled = scaler.transform(X_test)  # Test'i sadece transform et
    ...
    if saved_hyperparams is not None:
        ...
        for i in range(saved_hyperparams['n_layers']):
            print(f"  Katman {i+1} - Units: {saved_hyperparams[f'units_{i}']}, Dropout: {saved_hyperparams[f'dropout_{i}']:.2f}")
        ...
        first_fold = False  # Tuning'i atla
    ...
    for train_idx, val_idx in tscv.split(X_train_bal):
        ...
        if first_fold:
            ...
            print("\n🔍 Hyperparameter tuning başlıyor (TimeSeriesSplit ile)...")
            print("⏳ Bu işlem biraz zaman alabilir...\n")
            
            tuner = kt.Hyperband(
                lambda hp: build_model(hp, X_tr.shape[1]),
                objective='val_accuracy',
                max_epochs=30,
                factor=3,
                directory='tuner_results',
                project_name='mlp_ddos_tuning',
                overwrite=True
            )
            ...
            for i in range(best_hps.get('n_layers')):
                print(f"  Katman {i+1} - Units: {best_hps.get(f'units_{i}')}, Dropout: {best_hps.get(f'dropout_{i}'):.2f}")
        ...
        model = build_model(best_hps, X_tr.shape[1])
    ...
    final_model = build_model(best_hps, X_train_bal.shape[1])
    ...
    print("Final model eğitimi başlıyor (chronological validation)...")
    final_model.fit(
    ...
    test_metrics = print_detailed_metrics(
        ...
        test_preds, 
        test_preds_proba,
        dataset_name="Test Set (Ana Veri Seti)"
    )
    
    print("Detaylı Classification Report:")
    print(classification_report(y_test, test_preds, target_names=["BENIGN(0)", "ATTACK(1)"]))

    # Genelleme testi - Farklı CSV dosyaları üzerinde test
    # if generalization_csvs:
    #     print(f"\n{'='*60}")
    #     print("GENELLEME TESTİ - Farklı Veri Setleri")
    #     print(f"{'='*60}\n")
        
    #     for gen_csv in generalization_csvs:
    #         if not os.path.exists(gen_csv):
    #             print(f"⚠️  Dosya bulunamadı: {gen_csv}\n")
    #             continue
            
    #         print(f"\n>>> Test ediliyor: {os.path.basename(gen_csv)}")
    #         try:
    #             X_gen, y_gen, _ = load_and_prepare_data(
    #                 gen_csv, label_column, available_features, verbose
    #             )
                
    #             # Aynı scaler ile transform et (train'den fit edilmiş)
    #             X_gen_scaled = scaler.transform(X_gen)
                
    #             # Tahmin yap
    #             gen_preds_proba = final_model.predict(X_gen_scaled, verbose=0).ravel()
    #             gen_preds = (gen_preds_proba >= 0.5).astype(int)
                
    #             # Metrikleri yazdır
    #             gen_metrics = print_detailed_metrics(
    #                 y_gen,
    #                 gen_preds,
    #                 gen_preds_proba,
    #                 dataset_name=f"Genelleme - {os.path.basename(gen_csv)}"
    #             )
    ...
    # return final_model, scaler, test_metrics

def main():
    # Argparse kaldırıldı; sabitlerden çağır
    preprocess_and_model_single_csv(
        csv_path=CSV_PATH,
